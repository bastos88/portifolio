...
export default function PixelProgressBar({ value, label, size = "default" }: PixelProgressBarProps) {
  ...
  return (
    <div className={`flex items-center ${isHud ? "gap-4" : "gap-3"}`}>
      {label ? (
      ...
      <span className={`${isHud ? "w-12 text-[11px] xl:text-[12px]" : "w-11 text-[10px]"} text-right font-pixel text-arcade-text`}>{value}%</span>
    </div>
  );
}
